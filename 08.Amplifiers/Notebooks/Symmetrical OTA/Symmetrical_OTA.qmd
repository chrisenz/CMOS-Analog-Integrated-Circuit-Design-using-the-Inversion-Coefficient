...
license: CC BY-NC-SA
abstract: |
  This notebook presents the analysis, design and simulation of the symmetrical cascode OTA.
execute:
  echo: false
  warning: false
toc: true
...
code-links:
  - text: Notebook code
    icon: github
    href: https://github.com/chrisenz/CMOS-Analog-Integrated-Circuit-Design-using-the-Inversion-Coefficient/tree/main/08.Amplifiers/Notebooks/Symmetrical%20OTA
format: 
  html:
  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \cohead[Design of the Symmetrical OTA]{Design of the Symmetrical OTA}
...
![Schematic of the symmetrical differential OTA.](Figures/Symmetrical_OTA_schematic){width=70% #fig-symmetrical_ota_schematic}
...
![Small-signal schematic of the symmetrical cascode OTA.](Figures/Symmetrical_OTA_small-signal){width=70% #fig-symmetrical_ota_small-signal}

The small-signal schematic of the symmetrical OTA of @fig-symmetrical_ota_schematic is shown in @fig-symmetrical_ota_small-signal. The design of the symmetrical cascode OTA is similar to the design of the simple OTA, except that there are now more parasitic poles appearing at the various current mirror nodes. The small-signal output voltage in open-loop is given by
\begin{align}
...
\begin{equation}
  \begin{split}
    f \cdot R_{nfl} =& \frac{2 \rho_n}{W_{eff1}\,L_{eff1}} + \rho_p\,\left(\frac{G_{m2}}{G_{m1}}\right)^2 \left(\frac{2}{W_{eff2}\,L_{eff2}}+\frac{1}{W_{eff2c}\,L_{eff2c}}+\frac{1}{W_{eff2d}\,L_{eff2d}}\right)\\
    &+ \frac{\rho_n}{A_1^2}\left(\frac{G_{m3}}{G_{m1}}\right)^2\left(\frac{1}{W_{eff3a}\,L_{eff3a}}+\frac{1}{W_{eff3b}\,L_{eff3b}}\right)
  \end{split}
\end{equation}
which can be written as
\begin{equation}
  R_{nfl} = \frac{2 \rho_n}{W_{eff1}\,L_{eff1} f} \cdot (1 + \eta_{fl})
\end{equation}
...
\begin{equation}
  \begin{split}
    \eta_{fl} =& \frac{\rho_p}{2\rho_n}\,\left(\frac{G_{m2}}{G_{m1}}\right)^2 \left(\frac{2 W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}}+\frac{W_{eff1}\,L_{eff1}}{W_{eff2c}\,L_{eff2c}}+\frac{W_{eff1}\,L_{eff1}}{W_{eff2d}\,L_{eff2d}}\right)\\
    &+ \frac{1}{2 A_1^2}\left(\frac{G_{m3}}{G_{m1}}\right)^2\left(\frac{W_{eff1}\,L_{eff1}}{W_{eff3a}\,L_{eff3a}}+\frac{W_{eff1}\,L_{eff1}}{W_{eff3b}\,L_{eff3b}}\right).
  \end{split}
\end{equation}

For $A_1=A_2=A_3=1$, $W_{2a}=W_{2b}=W_{2c}=W_{2d}=W_2$, $L_{2a}=L_{2b}=L_{2c}=L_{2d}=L_2$, $W_{3a}=W_{3b}=W_3$ and $L_{3a}=L_{3b}=L_3$ reduces to
\begin{equation}
  \eta_{fl} = 2\frac{\rho_p}{\rho_n}\,\left(\frac{G_{m2}}{G_{m1}}\right)^2\,\frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \left(\frac{G_{m3}}{G_{m1}}\right)^2\,\frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}.
\end{equation}
...
\begin{equation}
  f_k = \frac{\rho_n\,G_{m1}}{W_{eff1}\,L_{eff1}\,\gamma_{n1}} \cdot \frac{1+\eta_{fl}}{1+\eta_{th}}.
\end{equation}
...
with
\begin{align}
  \sigma_{V_{T1}}^2 &= \frac{A_{VTn}^2}{W_{eff1}\,L_{eff1}},\\
  \sigma_{V_{T2}}^2 &= \frac{A_{VTp}^2}{W_{eff2}\,L_{eff2}},\\
  \sigma_{V_{T3}}^2 &= \frac{A_{VTn}^2}{W_{eff3}\,L_{eff3}},
\end{align}
and
\begin{align}
  \sigma_{\beta_1}^2 &= \frac{A_{\beta n}^2}{W_{eff1}\,L_{eff1}},\\
  \sigma_{\beta_2}^2 &= \frac{A_{\beta p}^2}{W_{eff2}\,L_{eff2}},\\
  \sigma_{\beta_3}^2 &= \frac{A_{\beta n}^2}{W_{eff3}\,L_{eff3}}.
\end{align}

Replacing in \eqref{eqn:xivt1} and \eqref{eqn:xibeta1} results in
\begin{equation}\label{eqn:xivt2}
  \xi_{V_T} = 2 \left(\frac{G_{m2}}{G_{m1}}\right)^2 \cdot \left(\frac{A_{VTp}}{A_{VTn}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \left(\frac{G_{m3}}{A_1\,G_{m1}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}
\end{equation}
and 
\begin{equation}\label{eqn:xibeta2}
  \xi_{\beta} = 2 \left(\frac{A_{\beta p}}{A_{\beta n}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \frac{1}{A_1^2}\,\frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}
\end{equation}

Similarly to the flicker noise, the input-referred offset (variance or standard deviation) can be reduced by increasing the differential pair M~1a~-M~1b~ area $W_{eff1}\,L_{eff1}$ but at the same time also increasing the area of the current mirrors $W_{eff2}\,L_{eff2}$ (and hence $W_{2c} L_{2c}$ and $W_{2d} L_{2d}$) and depending on $A_1$ also the area of the current mirror M~3a~-M~3b~ $W_{3a} L_{3a}$ and $W_{3b} L_{3b}$. This obviously comes at the cost of increased parasitic capacitances at nodes 2, 3 and 4 that will unavoidably reduce the phase margin.
...
## Process
We will design the symmetrical cascode OTA for a generic 180nm bulk CMOS process. The physical parameters are given in @tbl-physics_param, the global process parameters in @tbl-process_param and finally the MOSFET parameters in @tbl-mos_param. 
...
The gate voltage of M~2a~-M~2c~ and M~2a~-M~2b~ should be set as low as possible for a given maximum common mode input voltage $V_{ic,max}$ still keeping M~1a~-M~1b~ in saturation. The source-to-gate voltage of M~2a~-M~2b~ $V_{SG2}$ is given by
\begin{equation}
...
CW=2*(CGSOp+CGBOp)
Weff2=(-CW*WoverL2+sqrt(WoverL2)*sqrt(4*C2*CWL+CW**2*WoverL2))/(2*CWL)
...
\end{equation}
where $C_{GSop}$ is the gate-to-source overlap capacitance per unit width for pMOS transistors. $c_{gsi}$ is the intrinsic gate-to-source capacitance normalized to the total gate capacitance $W_{eff}\,L_{eff}\,C_{ox}$, which is typically equal to $2/3$ in strong inversion and is proportionnal to $IC$ in weak inversion.

The gate-to-bulk capacitance $C_{GB2}$ is given by
\begin{equation}
  C_{GB2} \cong \,W_{eff2}\,L_{eff2}\,C_{ox} \cdot c_{gbi} + C_{GBop} \cdot W_{eff2},
\end{equation}
where $C_{GBop}$ is the gate-to-bulk overlap capacitance per unit width and $c_{gbi}$ is the gate-to-bulk intrinsic capacitance normalized to the total gate capacitance $W_{eff}\,L_{eff}\,C_{ox}$ and given by
\begin{equation}
  c_{gbi} = \frac{n-1}{n} \cdot c_{gsi}.
\end{equation}

The capacitance at node 2 then scales with $W_{eff2}$ and $L_{eff2}$ according to
\begin{equation}
  C_2 = W_{eff2}\,L_{eff2} \cdot C_{WL} + W_{eff2} \cdot C_W,
\end{equation}
with
\begin{align}
...
We already have set the inversion coefficient $IC_2 =$ `{python} f'{IC2:.3f}'`, from which we can calculate $c_{gsi} =$ `{python} f'{cgsi_ic(IC2):.3f}'` and $c_{gbi} =$ `{python} f'{cgbi_ic(IC2,n0p):.3f}'`. Since the aspect ratio $AR_2 \triangleq W_{eff2}/L_{eff2}$ has already been set by the transconductance and the current, we can derive $W_{eff2}$ and $L_{eff2}$ for achieving a given capacitance $C_2$ according to
\begin{align}
  W_{eff2} &= \frac{-C_W \cdot AR_2 + \sqrt{AR_2} \cdot \sqrt{4\,C_2\,C_{WL}+C_W^2 \cdot AR_2}}{2\,C_{WL}},\\
  L_{eff2} &= \frac{W_{eff2}}{AR_2}.
\end{align}

Setting the non-dominant pole $f_{p2}$ to `{python} f'{fp2GBW:.0f}'` times the $GBW$, we get $C_2 =$ `{python} f'{C2/1e-15:.0f}'` fF, $L_{eff2} =$ `{python} f'{Leff2/1e-6:.2f}'` $\mu m$, $L_2 =$ `{python} f'{L2/1e-6:.2f}'` $\mu m$ and $W_{eff2} =$ `{python} f'{Weff2/1e-9:.0f}'` $nm$, $W_2 =$ `{python} f'{W2/1e-9:.0f}'` $nm$.
...
Knowing the current and the inversion coefficient $IC_3 =$ `{python} f'{IC3:.1f}'`, we can deduce $I_{spec3} =$ `{python} f'{Ispec3/1e-9:.3f}'` $nA$, $W_3/L_3 =$ `{python} f'{WoverL3:.1e}'` and $G_{m3} =$ `{python} f'{Gm3/1e-6:.3f}'` $\mu A/V$.
...
CW3=2*(CGSOn+CGBOn)
Weff3=(-CW3*WoverL3+sqrt(WoverL3)*sqrt(4*C3*CWL3+CW3**2*WoverL3))/(2*CWL3)
W3=Weff3-DWn
W3=round(1E8*W3)/1E8
Leff3=Weff3/WoverL3
L3=Leff3-DLn
...
def fic(IC):
    ...
    Weff=(-CW*WoverL+sqrt(WoverL)*sqrt(4*C3*CWL+CW**2*WoverL))/(2*CWL)
    W=Weff-DWn
#    print(W)
    return W-Wmin
...
for k in range(0,Npts):
    ...
    WoverL=Ispec/Ispecsqn
    C3=Gm/wp3
    Weff[k]=(-CW*WoverL+sqrt(WoverL)*sqrt(4*C3*CWL+CW**2*WoverL))/(2*CWL)
    Leff[k]=Weff[k]/WoverL

plt.loglog([ICmin,IC3],[Weffmin/1e-9,Weffmin/1e-9], 'k--', linewidth=lw)
plt.loglog([IC3,IC3],[1e-7,Weffmin/1e-9], 'k--', linewidth=lw)
plt.loglog(IC, Weff/1e-9, 'r-', label='$W_3$')
plt.loglog(IC, Leff/1e-9, 'b-', label='$L_3$')
plt.xlim(ICmin,ICmax)
#plt.xticks(np.arange(0,24,2))
plt.xlabel('$IC_3$')
plt.ylim(100,1e5)
#plt.yticks(np.arange(0,22,2))
plt.ylabel('$W_{eff3}$ and $L_{eff3}$ [nm]')
#plt.legend(loc='lower left')
plt.legend(loc='upper left')
textstr = f'$f_{{p3}} =$ {fp3/1e6:.1f} MHz'
plt.text(0.7, 0.95, textstr, ha='left', va='top', size=textFontSize, transform=plt.gca().transAxes,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
plt.text(0.9, Weffmin/1e-9, f'$W_{{eff,min}} =$ {Weffmin/1e-9:.0f}', ha='right', va='center', size=textFontSize)
plt.text(IC3, 85, f'$IC_{{3,opt}} =$ {IC3:.1f}', ha='left', va='top', size=textFontSize)
plt.show()
```
...
Gds3=Ib/(lambdan*Leff3)
Gds7=Gms7/Gds3*Go/2
Leff7=Ib/(lambdan*Gds7)
L7=Leff7-DLn
L7=round(1E8*L7)/1E8
Weff7=WoverL7*Leff7
W7=Weff7-DWn
...
To have some margin and avoid DIBL, we choose a slightly longer length $L_7 =$ `{python} f'{L7/1e-9:.0f}'` $nm$ ($L_{eff7} =$ `{python} f'{Leff7/1e-9:.0f}'` $nm$). Keeping the same $W_7/L_7 =$ `{python} f'{WoverL7:.1f}'`, we get $W_7 =$ `{python} f'{W7/1e-6:.2f}'` $\mu m$ ($W_{eff7} =$ `{python} f'{Weff7/1e-6:.2f}'` $\mu m$).

The minimum value of the bias voltage $V_{b2}$ is set such that M~3b~ remains in saturation
\begin{equation}
  V_{b2,min} = V_{DSsat3} + V_{GS2}.
\end{equation}
The saturation voltage of M~3b~ is equal to $V_{DSsat3} =$ `{python} f'{VDSsat3/1e-3:.0f}'` $mV$ and the $V_{GS}$ voltage of M~7~ is $V_{GS7} \cong$ `{python} f'{VGS7/1e-3:.0f}'` $mV$, which results $V_{b2,min} =$ `{python} f'{Vb2min/1e-3:.0f}'` $mV$. We finally choose $V_{b2} = V_{DD}/2 =$ `{python} f'{Vb2/1e-3:.0f}'` $mV$.
...
We could choose $L_4 = L_{min}$, but in order to have some margin on the DC gain, we choose $L_4 =$ `{python} f'{L4/1e-9:.0f}'` $nm$ ($L_{eff4} =$ `{python} f'{Leff4/1e-9:.0f}'` $nm$). Keeping the same $W_4/L_4 =$ `{python} f'{WoverL4:.1f}'`, this leads to $W_4 =$ `{python} f'{W4/1e-6:.2f}'` $\mu m$ ($W_{eff4} =$ `{python} f'{Weff4/1e-6:.2f}'` $\mu m$).
...
\end{equation}
The saturation voltage of M~2d~ is $V_{SDsat2} =$ `{python} f'{VSDsat2/1e-3:.0f}'` $mV$ and the source-to-gate voltage of M~4~ is $V_{SG4} \cong$ `{python} f'{VSG4/1e-3:.0f}'` $mV$, leading to $V_{b1,max} \cong$ `{python} f'{Vb1max:.3f}'` $V$. For convenience we could also set $V_{b1} = V_{DD}/2 =$ `{python} f'{Vb1/1e-3:.0f}'` $mV$. However, in the simulations below, we see that the output conductance of M~4~ is too large degrading the DC gain. This can be corrected by increasing the $V_{DS}$ voltage of M~4~ therefore reducing its output conductance allowing to reach the desired DC gain.

```{python}
# Check the resulting DC gain
Leff3=L3+DLn
Gds3b=Ib/(lambdan*Leff3)
Leff7=L7+DLn
Ispec7=Ispecsqn*Weff7/Leff7
IC7=Ib/Ispec7
Gms7=Ib/UT*gmsid_ic(IC7)
Gds7=Ib/(lambdan*L7)
Gon=Gds3b*Gds7/Gms7
Leff2=L2+DLp
Gds2d=Ib/(lambdap*Leff2)
Weff4=W4+DWp
Leff4=L4+DLp
...
Transistor M~5a~ is identical to M~5b~ and hence $W_{5a} = W_{5b} =$ `{python} f'{W5/1e-9:.0f}'` $nm$ ($W_{eff5} =$ `{python} f'{Weff5/1e-9:.0f}'` $nm$) and $L_{5a} = L_{5b} =$ `{python} f'{L5/1e-6:.2f}'` $\mu m$ ($L_{eff5} =$ `{python} f'{Leff5/1e-6:.2f}'` $\mu m$).
...
Weff1=sqrt(WoverL1*WL1)
W1=Weff1-DWn
W1=round(1E8*W1)/1E8
Leff1=sqrt(WL1/WoverL1)
L1=Leff1-DLn
L1=round(1E8*L1)/1E8
flickerpMOSCM=rhop/rhon*(Gm2/Gm1)**2*2*Weff1*Leff1/(Weff2*Leff2)
flickernMOSCM=(Gm3/Gm1)**2*Weff1*Leff1/(Weff3*Leff3)
...
etafl=2*rhop/rhon*(Gm2/Gm1)**2*Weff1*Leff1/(Weff2*Leff2)+(Gm3/Gm1)**2*Weff1*Leff1/(Weff3*Leff3)
...
\begin{equation}
  \eta_{fl} = 2\,\frac{\rho_p}{\rho_n} \cdot \left(\frac{G_{m2}}{G_{m1}}\right)^2\,\frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \left(\frac{G_{m3}}{G_{m1}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}.
\end{equation}
...
Since we already have $W_1/L_1$ we can deduce $W_1 =$ `{python} f'{W1/1e-6:.2f}'` $\mu m$ ($W_{eff1} =$ `{python} f'{Weff1/1e-6:.2f}'` $\mu m$) and $L_1 =$ `{python} f'{L1/1e-9:.0f}'` $nm$ ($L_{eff1} =$ `{python} f'{Leff1/1e-9:.0f}'` $nm$). Since transistor M~1a~ is identical to M~1b~, we have $W_{1a} = W_{1b} =$ `{python} f'{W1/1e-6:.2f}'` $\mu m$ and $L_{1a} = L_{1b} =$ `{python} f'{L1/1e-9:.0f}'` $nm$.
...
axs[0].set(xlim=(fmin,fmax), ylim=(-60,12), ylabel='$|A_{vd}|$ [dB]')
axs[0].set_yticks([-60,-40,-20,0,20,40,60,80,100,120])
#axs[0].legend(loc='best')
textstr1 = '\n'.join((
    f'$A_{{dc}} =$ {AdcdBthe:.1f} dB',
    f'$GBW =$ {GBWthe/1e6:.1f} GHz'))
axs[0].text(0.7, 0.93, textstr1, ha='left', va='top', transform=axs[0].transAxes, size=textFontSize,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
...
Weff3=ota_df.at['M3a','Weff']
Leff3=ota_df.at['M3a','Leff']
etafl=2*rhop/rhon*(Gm2/Gm1)**2*Weff1*Leff1/(Weff2*Leff2)+(Gm3/Gm1)**2*Weff1*Leff1/(Weff3*Leff3)
Rnfl1Hz=2*rhon/(Weff1*Leff1)*(1+etafl)
Sfl1Hz=4*kT*Rnfl1Hz
Sninfl1HzdBv=10*log(Sfl1Hz)
fkthe=2*rhon/(Weff1*Leff1*Rninththe)*(1+etafl)
table = [["$(G_{m1}/G_{m2})^2$",f'{(Gm1/Gm2)**2:.1f}',"-"],
         ...
         ["$\\frac{{W_{{eff1}} \\cdot L_{{eff1}}}}{{W_{{eff3}} \\cdot L_{{eff3}}}}$",f'{Weff1*Leff1/(Weff3*Leff3):.1f}',"-"],
         ["$\\eta_{fl}$",f'{etafl:.3f}',"-"],
...
\begin{equation}
  \xi_{V_T} = 2\left(\frac{G_{m2}}{A_1\,G_{m1}}\right)^2 \cdot \left(\frac{A_{VTp}}{A_{VTn}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \left(\frac{G_{m3}}{G_{m1}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}
\end{equation}
...
\begin{equation}
  \xi_{\beta} = 2\left(\frac{A_{\beta p}}{A_{\beta n}}\right)^2 \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff2}\,L_{eff2}} + \frac{1}{A_1^2} \cdot \frac{W_{eff1}\,L_{eff1}}{W_{eff3}\,L_{eff3}}.
\end{equation}
with
\begin{align}
  \sigma_{V_{T1}}^2 &= \frac{A_{VTn}^2}{W_{eff1}\,L_{eff1}},\\
...
\begin{align}
  ...
  \sigma_{\beta 2}^2 &= \frac{A_{\beta p}^2}{W_{eff2}\,L_{eff2}},\\
  \sigma_{\beta 3}^2 &= \frac{A_{\beta n}^2}{W_{eff3}\,L_{eff3}}.
\end{align}
...
plt.plot([Vidmin/1e-6,Vos/1e-6],[Vic,Vic],'k--', linewidth=lw)
plt.plot([Vos/1e-6,Vos/1e-6],[0.6,Vic],'k--', linewidth=lw)
plt.plot([Vidmin/1e-6,Vidmax/1e-6],[Voutmax,Voutmax],'k--', linewidth=lw)
plt.plot([Vidmin/1e-6,Vidmax/1e-6],[Voutmin,Voutmin],'k--', linewidth=lw)
plt.plot([24.2,25.7],[Voutmin,Voutmax],'k--', linewidth=lw)
Vswing=Voutmax-Voutmin
plt.plot(Vidsim/1e-6,Voutsim,'r')
plt.xlim(Vidmin/1e-6,Vidmax/1e-6)
...
From @fig-ng_large_signal_zoom we also see that the linear region, where the gain is constant, is actually smaller resulting in a smaller output voltage swing $V_{out,swing} \cong$ `{python} f'{Vswing/1e-3:.0f}'` $mV$.
...
axs[0].text(0.63, 0.93, textstr1, ha='left', va='top', transform=axs[0].transAxes, size=textFontSize,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
...
plt.subplots_adjust(hspace=0.1)
if savePlots: saveFigures(savePlotPath, otaName + '_closed_loop_gain_sim')
plt.show()
plt.style.use(pltStyle)
```
...
From @fig-ng_noise, we see a perfect match between simulation and theoretical estimation for both the flicker noise and the white noise except a^bove the $GBW$ where the simulated noise PSD becomes larger than the theoretical estimation. This is simply due to the fact that the input-referred noise PSD has been obtain by dividing the output current noise by the square of the equivalent trnasconductance without accouting for the frequency dependence occuring above the $GBW$.
...
df_noise=pd.read_table(ngspiceRawFile, sep=r'\s+', skiprows=(41), skip_blank_lines=True, header=None, dtype=np.float64, engine='python')
...
VnoutM4w=np.zeros(Npts)
VnoutM4f=np.zeros(Npts)
VnoutM7=np.zeros(Npts)
VnoutM7w=np.zeros(Npts)
...
for k in range(0,Npts):
  ...
  VnoutM2d[k]=df_noise.iloc[Nvar*k+17,0]
  VnoutM2df[k]=df_noise.iloc[Nvar*k+18,0]
  VnoutM2dw[k]=df_noise.iloc[Nvar*k+19,0]
  VnoutM3a[k]=df_noise.iloc[Nvar*k+20,0]
  VnoutM3af[k]=df_noise.iloc[Nvar*k+21,0]
  VnoutM3aw[k]=df_noise.iloc[Nvar*k+22,0]
  VnoutM3b[k]=df_noise.iloc[Nvar*k+23,0]
  VnoutM3bf[k]=df_noise.iloc[Nvar*k+24,0]
  VnoutM3bw[k]=df_noise.iloc[Nvar*k+25,0]
  VnoutM4[k]=df_noise.iloc[Nvar*k+26,0]
  VnoutM4f[k]=df_noise.iloc[Nvar*k+27,0]
  VnoutM4w[k]=df_noise.iloc[Nvar*k+28,0]
  VnoutM7[k]=df_noise.iloc[Nvar*k+29,0]
  VnoutM7f[k]=df_noise.iloc[Nvar*k+30,0]
  VnoutM7w[k]=df_noise.iloc[Nvar*k+31,0]
  Vnout[k]=df_noise.iloc[Nvar*k+32,0]
  ...
  SnoutM2w[k]=VnoutM2aw[k]**2+VnoutM2bw[k]**2+VnoutM2cw[k]**2+VnoutM2dw[k]**2
  ...
  SnoutM4w[k]=VnoutM4w[k]**2
  SnoutM7[k]=VnoutM7[k]**2
  SnoutM7f[k]=VnoutM7f[k]**2
  SnoutM7w[k]=VnoutM7w[k]**2
  A[k]=Vnout[k]/Vnin[k]
  Snin[k]=Vnin[k]**2
  ...
  Sninw[k]=SninM1w[k]+SninM2w[k]+SninM3w[k]+SninM4w[k]+SninM7w[k]
  SninfdBv[k]=10*log(Sninf[k])
  SninwdBv[k]=10*log(Sninw[k])

S0sim=Sninw[0]
S0dBvsim=10*log(S0sim)
...
SninM3w1ksim=SninM3w[np.where(freq > 1000)[0][0]]
...
gammanotasim=df_mos.at["M1a","Gm"]*Rninthsim
gamman2sim=df_mos.at["M2a","gamman"]
...
From @fig-ng_white_noise_contributions, we can observe a perfect match of the total input-referred white noise PSD between simulation and the theoretical estimation. The white noise is dominated by the differential pair M~1a~-M~1b~ which is $1+\eta_{th} =$ `{python} f'{1+etathsim:.3f}'` times (or `{python} f'{10*log(abs(1+etathsim)):.3f}'` dB) lower than the total white noise. The contribution of M~2a~-M~2b~-M~2c~-M~2d~ is about `{python} f'{SninM1w1ksim/SninM2w1ksim:.3f}'` times (or  `{python} f'{10*log(SninM1w1ksim/SninM2w1ksim):.3f}'` dB) lower than the contribution of M~1a~-M~1b~. The contribution of M~3a~-M~3b~ is about `{python} f'{SninM1w1ksim/SninM3w1ksim:.3f}'` times (or  `{python} f'{10*log(SninM1w1ksim/SninM3w1ksim):.3f}'` dB) lower than the contribution of M~1a~-M~1b~. Finally, the contributions of the cascode transistors M~4~ and M~7~ are completely negligible. (`{python} f'{10*log(SninM1w1ksim/SninM4w1ksim):.3f}'` dB lower for M~4~ and `{python} f'{10*log(SninM1w1ksim/SninM7w1ksim):.3f}'` dB lower for M~7~). This confirms that the noise of the cascode transistors M~4~ and M~7~ can be totally neglected.

The simulated value of $\eta_{th} =$ `{python} f'{etathsim:.3f}'` is almost equal to the theoretical estimation $\eta_{th} =$ `{python} f'{etath:.3f}'`. Finally, the simulated OTA thermal noise excess factor $\gamma_{n,ota} =$  `{python} f'{gammanotasim:.3f}'` is slightly larger than the predicted value $\gamma_{n,ota} =$  `{python} f'{gammanotathe:.3f}'`.
...
The breakdown of the contributions of the various transistors to the total input-referred noise is presented in @fig-ng_noise_contributions. We can observe that the simulation is slightly larger than the theoretical estimation.
...
Voutsim=df_sim['Vout'].to_numpy()
plt.plot([0,1.6],[0,1.6],'k--', linewidth=lw)
plt.plot([0,1.45],[1.45,1.45],'k--', linewidth=lw)
plt.plot([1.45,1.45],[0,1.45],'k--', linewidth=lw)
...
As shown in @fig-ng_cm_range, the output follows the input voltage up to `{python} f'{Vcmmax:.2f}'` $V$. So the input common-mode voltage range is about `{python} f'{Vcmmax:.2f}'` $V$.
